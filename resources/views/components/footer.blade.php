<div class="max-w-6xl mx-auto flex justify-center py-6 font-semibold">
{{--    <div class="px-8 border-r-2 border-red-theme text-lg">HOME</div>--}}
    <div class="px-8 border-r-2 border-red-theme text-lg"><a href="/">TOURS</a></div>
{{--    <div class="px-8 border-r-2 border-red-theme text-lg">COMPANY</div>--}}
    <div class="px-8 border-r-2 border-red-theme text-lg"><a href="/">MORE</a></div>
    <div class="px-8 text-lg">CALL US: <a href="tel:+17863039208">+1 786 3039208</a></div>
</div>
<div class="bg-footer-slider h-2.5"></div>
<footer class="bg-black">
    <div class="max-w-6xl mx-auto py-4 px-4 sm:px-6 flex">
        <div class="w-1/3 text-left text-white text-xs flex items-center">
            <span>©2021 Veza Sur On Tour. Privacy Policy.</span>
        </div>
        <div class="w-1/3 text-center">
            <img src="/images/logo.png" alt="Logo" class="h-8 mx-auto">
        </div>
        <div class="w-1/3 text-right flex items-center">
{{--            <img src="/images/logo_koi_footer.png" alt="Koi Footer" class="h-5 ml-auto">--}}
        </div>
    </div>
</footer>
